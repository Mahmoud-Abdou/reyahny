    ...
    <section id="data-thumb-view" class="data-thumb-view-header">
        ...
        <div class="table-responsive">
            <table id="sliders-table" class="table data-thumb-view">
                <thead>
                    <tr>
                        @can("reviews.c.user_name")
                            <th>{{ __('lang.user_name') }}</th>
                        @endcan
                        @can("reviews.c.vendor_name")
                            <th>{{ __('lang.vendor_name') }}</th>
                        @endcan
                        @can("reviews.c.feedback")
                            <th>{{ __('lang.feedback') }}</th>
                        @endcan
                        @can("reviews.c.rate")
                            <th>{{ __('lang.rate') }}</th>
                        @endcan
                        @can("reviews.c.anotheraction")
                            <th>{{ __('lang.action') }}</th>
                        @endcan
                        @can("reviews.c.action")
                            <th>#</th>
                        @endcan
                    </tr>
                </thead>
                    ...
                    <div class="data-fields px-2 mt-3">
                        <div class="row">
                            <div class="col-sm-12 data-field-col">
                                ...
                                <input type="text" class="form-control solid_up_required" id="edit-feedback">

                            </div>
                            <div class="col-sm-12 data-field-col">
                                <label for="edit-card_number">{{ __('lang.action') }}</label>
                                <input type="text" class="form-control solid_up_required" id="edit-action">

                            </div>

                            <input type="hidden" id="edit-review_id" name="edit_product_option_id">
...
<script>
                ...
                "columns": [
                    ...
                    @can("reviews.c.action")
                        {"data": function(data){
                            ...
                            <span class="action-edit" onclick="editNews(${data.id},'${data.action}','${data.feedback}')">
                                <i class="feather icon-edit" title="{{__('lang.delete')}}"></i>
                            </span>
   ...
   // editNews(${data.id}, '${data.name}','${data.image}' , '${data.action_link}')">
    function editNews(id,action,feedback){

        $("#edit-review_id").val(unescape(id));
...
function filterReviews(vendor_id){
    // alert(vendor_id);
    $('#sliders-table').dataTable().fnDestroy();
        var table= $("#sliders-table")
            .DataTable({
                responsive: !1,
                columnDefs: [
                ...
                "ajax": `/get-reviews?vendor_id=${vendor_id}`,
                "columns": [
                { "data": "user_name" , "defaultContent": "" },
                { "data": "vendor_name" , "defaultContent": "" },
                { "data": "feedback" , "defaultContent": "" },
                ...
                {"data": function(data){
                   return `
                        ...
                        <span class="action-edit" onclick="deleteNews(${data.id})">
                            <i class="feather icon-trash" title="{{__('lang.edit')}}"></i>
                        </span>
                        <span class="action-edit" onclick="editNews(${data.id},'${data.action}','${data.feedback}')">
                            <i class="feather icon-edit" title="{{__('lang.delete')}}"></i>
                        </span>
