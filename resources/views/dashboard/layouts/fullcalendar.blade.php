...
function gmod(n, m){
  ...
  */
  ...
  function writeIslamicDate(date, adjustment) {
    ...
    var iMonthNames = ["{{ __('lang.Muharram') }}", "{{ __('lang.Safar') }}", "{{ __('lang.RabiulAwwal') }}", "{{ __('lang.RabiulAkhir') }}", "{{ __('lang.JumadalUla') }}", "{{ __('lang.JumadalAkhira') }}",
                       "{{ __('lang.Rajab') }}", "{{ __('lang.Shaban') }}", "{{ __('lang.Ramadan') }}", "{{ __('lang.Shawwal') }}", "{{ __('lang.DhulQaada') }}", "{{ __('lang.DhulHijja') }}"];
    var iDate = kuwaiticalendar(date, adjustment);
    // var outputIslamicDate = wdNames[iDate[4]] + ", " + iDate[5] + " " +
    //                         iMonthNames[iDate[6]] + " " + iDate[7] + " AH";
    var outputDay = iDate[5];
    if(outputDay> 1){
...
// Date for 1 Jan 2017
// console.log(writeIslamicDate(new Date(2019,4,14)));
// Date for 1 Jan 2017, adjust by -1 day
// console.log(writeIslamicDate(new Date(2019,4,14), -1));


function dayClick(dayId){
    var selectedDayDate = $("#"+ dayId).attr('data-date');
    $("#"+ dayId).toggleClass('bgcolordefault');
    if($("#"+ dayId).children("input").attr('name') == "day[]"){
        $("#"+ dayId).children("input").remove();
    }else{
        $("#"+ dayId).append( '<input type="hidden" name="day[]" value="'+ selectedDayDate +'">' );
    } 
}

function HalaqaClick(HalaqaId){
    var selectedDayDate = $("#"+ dayId).attr('data-date');
    $("#"+ dayId).toggleClass('bgcolordefault');
    if($("#"+ dayId).children("input").attr('name') == "day[]"){
        $("#"+ dayId).children("input").remove();
...
(function(t) {
    ...
    function me(n, r) {
        ...
        function a() {
            re = Q.theme ? "ui" : "fc", n.addClass("fc"), Q.isRTL ? n.addClass("fc-rtl") : n.addClass("fc-ltr"), Q.theme ? n.addClass("ui-widget") : n.addClass("fc-unthemed"), ie = t("<div style='direction:{{ __('lang.directiontext') }}' class='fc-view-container'/>").prependTo(n), ee = new ve(q, Q), ne = ee.render(), ne && n.prepend(ne), c(Q.defaultView), Q.handleWindowResize && (le = G(w, Q.windowResizeDelay), t(window).resize(le))
        }
    ...
    function ve(e, n) {
        function i() {
            var e = n.header;
            return f = n.theme ? "ui" : "fc", e ? g = t("<div class='fc-toolbar'/>").append(s("left")).append(s("center")).append('<div class="fc-clear"/>') : void 0
        }
    ...
    $e.mixin({
        ...
        fgSegHtml: function(t, e) {
            var n, i = this.view,
                ...
                u = "";
            return l.unshift("fc-day-grid-event"), !r.allDay && t.isStart && (u = '<span class="fc-time">' + z(this.getEventTimeText(r)) + "</span>"), n = '<span class="fc-title">' + (z(r.title || "") || "&nbsp;") + "</span>", '<a class="' + l.join(" ") + '"' + (r.url ? ' onclick="' + 'checkandopenmodal('+ (z(r.id || "") || "&nbsp;") +')' + '" href="' + z(r.url) + '"' : "") + (a ? ' style="' + a + '"' : "") + ">" + '<div class="fc-content">' + (this.isRTL ? n + " " + u : u + " " + n) + "</div>" + (o ? '<div class="fc-resizer"/>' : "") + "</a>"
        },
        ...
        //     var i = this,
        //         r = this.view;
        //     return t('<a value="'+ n.year()+n.month()+n.date() +'" class="fc-more"/>').text(this.getMoreLinkText(n.length)).on("click", function(s) {
    ...
    var Ke = {
        ...
        Je = Ce.basic = qe.extend({
            ...
            numberCellHtml: function(t) {
                var e, n = t.start;
                // +' - '+ writeIslamicDate(new Date(n.year(),n.month(),n.date()), -1)
                return this.dayNumbersVisible ? (e = this.dayGrid.getDayClasses(n), e.unshift("fc-day-number"), '<td class="' + e.join(" ") + '" data-date="' + n.format() +'" id="'+n.year()+n.month()+n.date()+ '" onclick="dayClick('+n.year()+n.month()+n.date()+')'+'">' + n.date() +"<span class='hijri-date'>" + writeIslamicDate(new Date(n.year(),n.month(),n.date()), -1) + "</span></td>") : "<td/>"
            },
